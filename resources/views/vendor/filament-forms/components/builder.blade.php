...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div x-data="{}"
                    ...
                    <li wire:key="{{ $this->getId() }}.{{ $item->getStatePath() }}.{{ $field::class }}.item"
                        ...
                        @if (
                            ...
                            <div @if ($isCollapsible) x-on:click.stop="isCollapsed = !isCollapsed" @endif
                                ...
                                @if ($hasBlockNumbers || $isCloneable || $isDeletable || $isCollapsible || count($visibleExtraItemActions))
                                    <ul class="ms-auto flex items-center gap-x-3">
                                        @if ($hasBlockNumbers)
                                            <li>
                                                <x-filament::badge class="inline-flex select-none pointer-events-none">
                                                    <strong>
                                                        #{{ $loop->iteration }}:
                                                        {{ $item->getParentComponent()->getDefaultLabel() }}
                                                    </strong>
                                                </x-filament::badge>
                                            </li>
                                        @endif
        ...
        @if ($isAddable)
            <x-filament-forms::builder.block-picker :action="$addAction" :blocks="$blockPickerBlocks" :columns="$blockPickerColumns"
                :state-path="$statePath" :width="$blockPickerWidth" class="flex justify-center">
                <x-slot name="trigger">
                    {{ $addAction }}
                </x-slot>
            </x-filament-forms::builder.block-picker>
